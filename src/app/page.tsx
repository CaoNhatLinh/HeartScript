...
export default function LoginPage() {
    ...
    return (
        <div className="relative w-full h-screen overflow-hidden bg-black flex flex-col md:flex-row font-sans">
            {/* Background Ambience */}
            ...
            {/* Error Message */}
            <div className="absolute top-0 w-full z-50 flex justify-center pt-8 pointer-events-none">
                <AnimatePresence>
                    {error && (
                        <motion.div
                            ...
                            exit={{ opacity: 0 }}
                            className="bg-red-500/90 backdrop-blur-md px-6 py-3 rounded-full text-white text-sm font-medium shadow-lg"
                        >
                            {error}
                        </motion.div>
                    )}
            ...
            <motion.div
                ...
                whileHover={{ flexGrow: 1.4, zIndex: 20 }}
            ...
            >
                ...
                <div className="text-center space-y-4 opacity-80 md:opacity-60 group-hover:opacity-100 transition-all transform group-hover:scale-110 duration-500 relative z-20">
                    <motion.div
                        animate={{ y: [0, -10, 0] }}
                        transition={{ duration: 4, repeat: Infinity, ease: "easeInOut" }}
                        className="w-24 h-24 bg-gradient-to-tr from-blue-600 to-cyan-500 rounded-full flex items-center justify-center mx-auto shadow-lg shadow-blue-500/30"
                    >
                        {loading === 'him' ? (
                            <Loader2 className="w-10 h-10 text-white animate-spin" />
                        ) : (
                            <Cloud className="w-10 h-10 text-white fill-white/20" />
                        )}
                    </motion.div>
                    <div>
                        <h2 className="text-4xl font-bold text-transparent bg-clip-text bg-gradient-to-r from-blue-100 to-indigo-200">Anh</h2>
                        <p className="text-blue-100/60 font-medium tracking-wide text-[10px] uppercase mt-1">Journey for Him</p>
                    </div>
                </div>
            </motion.div>
            ...
            >
                ...
                <div className="text-center space-y-4 opacity-80 md:opacity-60 group-hover:opacity-100 transition-all transform group-hover:scale-110 duration-500 relative z-20">
                    <motion.div
                        ...
                        className="w-24 h-24 bg-gradient-to-bl from-pink-500 to-rose-400 rounded-full flex items-center justify-center mx-auto shadow-lg shadow-pink-500/30"
                    >
                        {loading === 'her' ? (
                            <Loader2 className="w-10 h-10 text-white animate-spin" />
                        ) : (
                    ...
                    <div>
                        <h2 className="text-4xl font-bold text-transparent bg-clip-text bg-gradient-to-l from-pink-100 to-rose-200">Em</h2>
                        <p className="text-pink-100/60 font-medium tracking-wide text-[10px] uppercase mt-1">Journey for Her</p>
                    </div>
                </div>
            </motion.div>
