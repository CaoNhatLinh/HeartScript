...
export const FrameOverlay: React.FC = () => {
    ...
    // Show frame when visible (persists after selector closes)
    const isStarted = currentScene !== 'prelude';
    if (!isStarted || !frameVisible) return null;
            ...
            >
                ...
                {selectedFrame === 'cute' && (
                    <div className="w-full h-full max-w-[90vw] max-h-[80vh] border-[8px] border-dashed border-pink-300 rounded-[30px] bg-transparent relative">
                        <div className="absolute -top-4 left-8 text-pink-300"><Sparkles size={28} /></div>
                        <div className="absolute -bottom-4 right-8 text-pink-400"><Heart size={28} fill="currentColor" /></div>
                        <div className="absolute bottom-4 left-0 right-0 text-center">
