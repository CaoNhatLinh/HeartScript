...
export const FrameSelector: React.FC = () => {
    ...
    const frameVisible = useExperienceStore(s => s.frameVisible);
    const setFrameVisible = useExperienceStore(s => s.setFrameVisible);

    const handleSelect = (id: typeof selectedFrame) => {
        if (selectedFrame === id && frameVisible) {
            // Toggle off if same frame clicked again
    ...
    return (
        <div className="relative">
            ...
            <button
                ...
                title="Choose Frame"
                aria-label={isFrameSelectorOpen ? 'Đóng chọn khung' : 'Chọn khung viền'}
                aria-expanded={isFrameSelectorOpen}
            >
                    ...
                    >
                        ...
                        <div className="grid grid-cols-3 gap-3">
                            ...
                            <motion.button
                                ...
                                onClick={handleClear}
                                aria-label="Xóa khung viền"
                                className={`
                                    relative group flex flex-col items-center gap-1.5 p-2 rounded-lg transition-all
                                    ${!frameVisible ? 'ring-2 ring-white/40 ring-offset-2 ring-offset-black/80' : 'hover:bg-white/10'}
                                `}
                            >
                                <div className="w-16 h-16 rounded-lg overflow-hidden shadow-lg relative border-2 border-dashed border-white/20 flex items-center justify-center bg-white/5">
                                    <X size={20} className="text-white/40" />
                                </div>
                                <span className={`text-[10px] font-medium ${!frameVisible ? 'text-white/90' : 'text-white/70'}`}>
                                    None
                                </span>
                            </motion.button>
                            ...
                            {FRAMES.map((frame, index) => {
                                ...
                                const isSelected = selectedFrame === frame.id && frameVisible;
